...
def run():
    ...
    for ds in datasets:
        ...
        with console.status(f"[bold green]Reading {ds['file']}...", spinner="dots"):
            cols = [ds["y"]] + ds["exog"]
            if ds["endog"]:
                cols.append(ds["endog"])
            if ds["iv"]:
                cols.append(ds["iv"])
            
            # Read and drop nulls
            df = pl.read_csv(ds["file"], null_values=["."])
        ...
        y = pdf[ds["y"]]
        controls = ["const"] + ds["exog"]

        table = Table(title=f"Results: {ds['name']}", box=box.ROUNDED, header_style="bold cyan", border_style="dim")
        ...
        target_var = ds["endog"] if ds["endog"] else ds["exog"][0]

        # 1. OLS
        ols_exog = pdf[controls + ([ds["endog"]] if ds["endog"] else [])]
        ols_model = IV2SLS(dependent=y, exog=ols_exog, endog=None, instruments=None).fit(cov_type="robust")
        ...
        if ds["endog"] and ds["iv"]:
            iv_model = IV2SLS(dependent=y, exog=pdf[controls], endog=pdf[[ds["endog"]]], instruments=pdf[[ds["iv"]]]).fit(cov_type="robust")
            table.add_row(
                "[green]IV 2SLS[/green]", 
                target_var,
                f"[bold green]{iv_model.params[ds['endog']]:.4f}[/bold green]", 
                f"{iv_model.std_errors[ds['endog']]:.4f}", 
                f"{iv_model.tstats[ds['endog']]:.2f}", 
                f"{iv_model.pvalues[ds['endog']]:.4f}"
            )
